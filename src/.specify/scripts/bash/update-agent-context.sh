...
#    - Verifies feature context and required files
...
validate_environment() {
    ...
    if [[ -z "$FEATURE_SLUG" ]]; then
        ...
        log_info "Pass --feature <slug> to the setup script or create/select a feature first"
        exit 1
    fi
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "Set .specify/.current-feature or rerun the setup step with --feature <slug>"
        exit 1
    fi
...
update_existing_agent_file() {
    ...
    if [[ -n "$tech_stack" ]] && ! grep -q "$tech_stack" "$target_file"; then
        new_tech_entries+=("- $tech_stack ($FEATURE_SLUG)")
    fi

    if [[ -n "$NEW_DB" ]] && [[ "$NEW_DB" != "N/A" ]] && [[ "$NEW_DB" != "NEEDS CLARIFICATION" ]] && ! grep -q "$NEW_DB" "$target_file"; then
        new_tech_entries+=("- $NEW_DB ($FEATURE_SLUG)")
    fi

    # Prepare new change entry
    if [[ -n "$tech_stack" ]]; then
        new_change_entry="- $FEATURE_SLUG: Added $tech_stack"
    elif [[ -n "$NEW_DB" ]] && [[ "$NEW_DB" != "N/A" ]] && [[ "$NEW_DB" != "NEEDS CLARIFICATION" ]]; then
        new_change_entry="- $FEATURE_SLUG: Added $NEW_DB"
    fi
...
main() {
    ...
    log_info "=== Updating agent context files for feature $FEATURE_SLUG ==="
